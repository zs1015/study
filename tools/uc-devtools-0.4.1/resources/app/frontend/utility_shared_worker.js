...
_onMessage(event){this._messageHandler(event.data);}};var dispatchers=[];if(typeof SharedWorkerGlobalScope!='undefined'&&self instanceof SharedWorkerGlobalScope){function onNewPort(port){var dispatcher=new ServiceDispatcher(new WorkerServicePort(port));dispatchers.push(dispatcher);}
