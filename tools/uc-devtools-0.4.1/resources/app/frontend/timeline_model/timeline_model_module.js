...
return false;}
return true;}
static isMarkerEvent(event){const recordTypes=TimelineModel.TimelineModel.RecordType;switch(event.name){case recordTypes.TimeStamp:case recordTypes.MarkFirstPaint:case recordTypes.MarkT2:case recordTypes.FirstTextPaint:case recordTypes.FirstImagePaint:case recordTypes.FirstMeaningfulPaint:case recordTypes.FirstPaint:case recordTypes.FirstContentfulPaint:return true;case recordTypes.MarkDOMContent:case recordTypes.MarkLoad:return event.args['data']['isMainFrame'];default:return false;}}
...
startTime=endTime;}}
this._inspectedTargetEvents.sort(SDK.TracingModel.Event.compareStartTime);this._processBrowserEvents(tracingModel);this._buildTimelineRecords();this._buildGPUEvents(tracingModel);this._insertFirstPaintEvent();this._insertT2PaintEvent();this._resetProcessingState();}
...
_insertT2PaintEvent(){if(!this._first_uc_perf_start||!this._last_uc_perf_t2)
return;var t2event=this._last_uc_perf_t2;var startevent=this._first_uc_perf_start;var t2stamp=startevent.startTime+(t2event.args["data"]/1.0);if(!this._firstCompositeLayers)
return;var recordTypes=TimelineModel.TimelineModel.RecordType;var i=this._inspectedTargetEvents.lowerBound(this._firstCompositeLayers,SDK.TracingModel.Event.compareStartTime);for(;i<this._inspectedTargetEvents.length&&this._inspectedTargetEvents[i].name!==recordTypes.DrawFrame;++i){}
if(i>=this._inspectedTargetEvents.length)
return;var drawFrameEvent=this._inspectedTargetEvents[i];var T2Event=new SDK.TracingModel.Event('devtools.timeline',recordTypes.MarkT2,SDK.TracingModel.Phase.Instant,t2stamp,drawFrameEvent.thread);this._mainThreadEvents.splice(this._mainThreadEvents.lowerBound(T2Event,SDK.TracingModel.Event.compareStartTime),0,T2Event);var T2Record=new TimelineModel.TimelineModel.Record(T2Event);this._eventDividerRecords.splice(this._eventDividerRecords.lowerBound(T2Record,TimelineModel.TimelineModel.Record._compareStartTime),0,T2Record);}
...
threadEvents.push(event);if(event.name=='uc_perf_start'&&!this._first_uc_perf_start){this._first_uc_perf_start=event;}
if(event.name=='uc_perf_start'&&this._first_uc_perf_start){}
if(event.name=='uc_perf_t2'){this._last_uc_perf_t2=event;}
...
addChild(child){this.children.push(child);child.parent=this;}};TimelineModel.TimelineModel.PageFrame.mainFrameId='';TimelineModel.TimelineModel.ResourceSizeThresholds={'text/html':5*1024,'image/jpeg':30*1024,'image/png':30*1024,'application/javascript':10*1024,'text/css':5*1024,'text/plain':5*1024,'image/webp':30*1024,'text/javascript':10*1024,'application/json':10*1024,'image/jpg':30*1024,'application/x-javascript':10*1024,'image/gif':30*1024,'application/octet-stream':1024*1024,'image/svg+xml':30*1024,'application/x-font-woff':10*1024}
...
auditSize(){const thresholds=TimelineModel.TimelineModel.ResourceSizeThresholds;if(this.encodedDataLength&&this.mimeType&&this.encodedDataLength>thresholds[this.mimeType]){this.warning=TimelineModel.TimelineModel.WarningType.LargeSize;}}};TimelineModel.TimelineModel.Filter=class{accept(event){return true;}};TimelineModel.TimelineVisibleEventsFilter=class extends TimelineModel.TimelineModel.Filter{constructor(visibleTypes){super();this._visibleTypes=new Set(visibleTypes);}
