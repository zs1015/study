...
{var element=this._breakpointElements[url];var inputElement=createElementWithClass("span","breakpoint-condition editing");inputElement.textContent=url;this.listElement.insertBefore(inputElement,element);element.classList.add("hidden");function finishEditing(accept,e,text)
...
{var checkbox=createElementWithClass("input","checkbox-elem");checkbox.type="checkbox";labelNode.insertBefore(checkbox,labelNode.firstChild);return checkbox;},_categoryCheckboxClicked:function(categoryItem)
...
this._textEditor.markClean();this._updateStyle();WebInspector.notifications.dispatchEventToListeners(WebInspector.UserMetrics.UserAction,{action:WebInspector.UserMetrics.UserActionNames.FileSaved,url:this._uiSourceCode.url});},_onSavedStateUpdated:function(event)
...
WebInspector.UISourceCodeFrame.prototype.populateTextAreaContextMenu.call(this,contextMenu,lineNumber);if(this._uiSourceCode.project().type()===WebInspector.projectTypes.Network&&WebInspector.settings.jsSourceMapsEnabled.get()){if(this._scriptFileForTarget.size){var scriptFile=this._scriptFileForTarget.valuesArray()[0];var addSourceMapURLLabel=WebInspector.UIString(WebInspector.useLowerCaseMenuTitles()?"Add source map\u2026":"Add Source Map\u2026");contextMenu.appendItem(addSourceMapURLLabel,addSourceMapURL.bind(this,scriptFile));contextMenu.appendSeparator();}}},_workingCopyChanged:function(event)
...
return true;var scriptFiles=this._scriptFileForTarget.valuesArray();var hasDivergingOrMergingFile=false;for(var i=0;i<scriptFiles.length;++i)
...
{var nodes=this._uiSourceCodeNodes.valuesArray();for(var i=0;i<nodes.length;++i){var uiSourceCode=nodes[i].uiSourceCode();var inspectedPageURL=WebInspector.targetManager.inspectedPageURL();if(inspectedPageURL&&uiSourceCode.url===inspectedPageURL)
...
{var child=new TreeElement("",null,false);child.selectable=false;this.appendChild(child);var lineElement=createElement("span");function appendLineNumber(lineNumber)
...
promises.push(this._tabbedPaneController.viewForId(ids[i]));Promise.all(promises).then(filterNavigators.bind(this)).done();function filterNavigators(objects)
...
{this._watchObjectGroupId="watch-group";WebInspector.PropertiesSection.call(this,"");this.treeElementConstructor=WebInspector.WatchedPropertyTreeElement;this.skipProto=false;this._expandedExpressions=new Set();this._expandedProperties=new Set();this.emptyElement=createElementWithClass("div","info");this.emptyElement.textContent=WebInspector.UIString("No Watch Expressions");this.watchExpressions=WebInspector.settings.watchExpressions.get();this.headerElement.className="hidden";this.editable=true;this.expanded=true;this.propertiesElement.classList.add("watch-expressions");this.element.addEventListener("mousemove",this._mouseMove.bind(this),true);this.element.addEventListener("mouseleave",this._mouseLeave.bind(this),true);this.element.addEventListener("dblclick",this._sectionDoubleClick.bind(this),false);this.emptyElement.addEventListener("contextmenu",this._emptyElementContextMenu.bind(this),false);}
...
if(!propertyCount){this.element.appendChild(this.emptyElement);this.propertiesTreeOutline.removeChildren();this._expandedExpressions.clear();this._expandedProperties.clear();}else{this.emptyElement.remove();}
...
{WebInspector.ObjectPropertyTreeElement.prototype.onexpand.call(this);this.treeOutline.section._expandedExpressions.add(this._expression());},oncollapse:function()
{WebInspector.ObjectPropertyTreeElement.prototype.oncollapse.call(this);this.treeOutline.section._expandedExpressions.remove(this._expression());},onattach:function()
{WebInspector.ObjectPropertyTreeElement.prototype.onattach.call(this);if(this.treeOutline.section._expandedExpressions.has(this._expression()))
...
var deleteButton=createElementWithClass("input","enabled-button delete-button");deleteButton.type="button";deleteButton.title=WebInspector.UIString("Delete watch expression.");deleteButton.addEventListener("click",this._deleteButtonClicked.bind(this),false);this.listItemElement.addEventListener("contextmenu",this._contextMenu.bind(this),false);this.listItemElement.insertBefore(deleteButton,this.listItemElement.firstChild);},populateContextMenu:function(contextMenu)
{if(!this.isEditing()){contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles()?"Add watch expression":"Add Watch Expression"),this.treeOutline.section.addNewExpressionAndEdit.bind(this.treeOutline.section));contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles()?"Delete watch expression":"Delete Watch Expression"),this._deleteButtonClicked.bind(this,null));}
if(this.treeOutline.section.watchExpressions.length>1)
contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles()?"Delete all watch expressions":"Delete All Watch Expressions"),this._deleteAllButtonClicked.bind(this));if(!this.isEditing()&&(this.property.value.type==="number"||this.property.value.type==="string"))
contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles()?"Copy value":"Copy Value"),this._copyValueButtonClicked.bind(this));},_contextMenu:function(event)
...
{WebInspector.ObjectPropertyTreeElement.prototype.onattach.call(this);if(this.hasChildren&&this.treeOutline.section._expandedProperties.has(this.propertyPath()))
this.expand();},onexpand:function()
{WebInspector.ObjectPropertyTreeElement.prototype.onexpand.call(this);this.treeOutline.section._expandedProperties.add(this.propertyPath());},oncollapse:function()
{WebInspector.ObjectPropertyTreeElement.prototype.oncollapse.call(this);this.treeOutline.section._expandedProperties.remove(this.propertyPath());},__proto__:WebInspector.ObjectPropertyTreeElement.prototype};WebInspector.ThreadsSidebarPane=function()
...
{var uiSourceCode=(event.data);this._updateButton(uiSourceCode);var path=uiSourceCode.project().id()+":"+uiSourceCode.path();if(this._isFormatableScript(uiSourceCode)&&uiSourceCode.url&&this._pathsToFormatOnLoad.has(path)&&!this._formattedPaths.get(path))
...
{if(this._button)
return this._button.element;this._sourcesView=sourcesView;this._sourcesView.addEventListener(WebInspector.SourcesView.Events.EditorSelected,this._editorSelected.bind(this));this._sourcesView.addEventListener(WebInspector.SourcesView.Events.EditorClosed,this._editorClosed.bind(this));this._button=new WebInspector.StatusBarButton(WebInspector.UIString("Pretty print"),"sources-toggle-pretty-print-status-bar-item");this._button.toggled=false;this._button.addEventListener("click",this._toggleFormatScriptSource,this);this._updateButton(sourcesView.currentUISourceCode());return this._button.element;},_isFormatableScript:function(uiSourceCode)
{if(!uiSourceCode)
...
{var formatter=WebInspector.Formatter.createFormatter(uiSourceCode.contentType());var highlighterType=WebInspector.SourcesView.uiSourceCodeHighlighterType(uiSourceCode);formatter.formatContent(highlighterType,content||"",innerCallback.bind(this));}
...
return;this._editorContainer.addUISourceCode(uiSourceCode);var currentUISourceCode=this._currentUISourceCode;if(currentUISourceCode&&currentUISourceCode.project().isServiceProject()&&currentUISourceCode!==uiSourceCode&&currentUISourceCode.url===uiSourceCode.url&&uiSourceCode.url){this._showFile(uiSourceCode);this._editorContainer.removeUISourceCode(currentUISourceCode);}},_uiSourceCodeRemoved:function(event)
...
{this._workspace.addMapping(networkUISourceCode,uiSourceCode,WebInspector.fileSystemWorkspaceBinding);this._suggestReload();}},mapNetworkToFileSystem:function(networkUISourceCode)
...
{if(confirm(WebInspector.UIString("Are you sure you want to remove network mapping?"))){this._workspace.removeMapping(uiSourceCode);this._suggestReload();}},_appendUISourceCodeMappingItems:function(contextMenu,uiSourceCode)
{if(uiSourceCode.project().type()===WebInspector.projectTypes.FileSystem){var hasMappings=!!uiSourceCode.url;if(!hasMappings)
contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles()?"Map to network resource\u2026":"Map to Network Resource\u2026"),this.mapFileSystemToNetwork.bind(this,uiSourceCode));else
...
contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles()?"Map to file system resource\u2026":"Map to File System Resource\u2026"),this.mapNetworkToFileSystem.bind(this,uiSourceCode));}},_appendUISourceCodeItems:function(event,contextMenu,target)
...
{if(uiLocation instanceof WebInspector.UILocation){WebInspector.SourcesPanel.instance().showUILocation(uiLocation);return Promise.resolve();}
...
{if(uiSourceCode instanceof WebInspector.UISourceCode){WebInspector.SourcesPanel.instance().showUISourceCode(uiSourceCode);return Promise.resolve();}
...
return;if(uiSourceCode.project().type()===WebInspector.projectTypes.FileSystem){var hasMappings=!!uiSourceCode.url;if(hasMappings)
